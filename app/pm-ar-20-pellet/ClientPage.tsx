...
import { 
  ...
  Flame, ArrowRight, Camera, Plus, X, ChevronLeft, ChevronRight, LayoutDashboard, Truck, Zap
} from "lucide-react";

// --- PELLET SMOKER DATA ---
const BASE_PRICE = 2395;
const STOCK_NUMBER = "PM-PELLET-AR-20x32"; 

const SPECS = [
  { label: "Main Chamber", value: "20” x 36”", icon: Ruler },
  { label: "Grate Area", value: "1216 Sq. In.", icon: LayoutDashboard },
  { label: "Controller", value: "Pellet Pro PID", icon: Zap },
...
const STANDARD_FEATURES = [
  ...
  "Fold up Steel Shelf (folds down for getting though doorways).",
  "Slide-Out Cooking Grates (Included).",
  "Dual Baffle Plate Above Heat Pot for Clean and Easy Smoking Meat.",
  "1-1/4″ Ball Valve Drain for Easy Cleanup.",
  "Fully Seam-Welded Flanged Lids (Not Stitch-Welded).",
  "Stainless Steel Cooking Chamber Hinges and 'Cool-to-Touch' Handles.",
  "Heavy Duty 7″ Casters for Easy Moving."
];

const CUSTOM_OPTIONS = [
  { id: "stainless-shelf", label: "Stainless Steel Top Upgrade for Shelf", price: 200, icon: Hammer, desc: "Upgrade the standard steel folding front shelf to a premium solid stainless steel top." }
];

export default function PelletClient({ galleryImages }: { galleryImages: string[] }) {
    ...
    <main className="min-h-screen bg-[#0a0a0a] selection:bg-[#EA580C] pt-24 pb-48 font-sans">
      ...
      <section className="relative container mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
        <div className="w-full lg:w-1/2 relative aspect-[4/3] rounded-[2.5rem] md:rounded-[3.5rem] bg-[#111111] border border-white/10 overflow-hidden shadow-2xl group">
          ...
          <Image 
            src="/images/smoker_pellet.webp" 
            alt="Pitmaker PM AR-20 Pellet Smoker" 
            fill 
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center">
          ...
          <h1 className="font-oswald text-5xl md:text-7xl font-black text-white uppercase leading-[0.9] mb-6">
            PM AR-20 <br /> <span className="text-zinc-500">Pellet</span>
          </h1>
          <p className="text-zinc-400 text-lg md:text-xl font-light leading-relaxed mb-8 max-w-xl">
            Set it, and forget it. Featuring a 1/4″ thick steel cooking chamber, the AR-20 is one of the most heat-retentive and strongest pellet grills in the industry. Powered by the best PID Controller on the market for pure, hassle-free wood smoke flavor.
          </p>
          ...
          <div className="lg:col-span-4 sticky top-32 z-20">
             <div className="bg-[#111111] rounded-[2.5rem] border border-white/10 shadow-2xl p-8 overflow-hidden relative">
                ...
                <div className="flex justify-between items-center mb-4 text-sm text-zinc-300">
                  <span>Base PM AR-20 Pellet</span>
      ...
      <section className="container mx-auto px-6 mt-32 border-t border-white/5 pt-20">
         <div className="flex items-center justify-between mb-12">
            <div>
               ...
               <p className="text-zinc-500 font-light text-sm">See the AR-20 Pellet in action.</p>
            </div>
         ...
         {galleryImages.length === 0 ? (
           <div className="w-full py-20 border border-dashed border-zinc-800 rounded-[2rem] flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-xs font-light mt-2">Upload images to <code className="bg-black px-2 py-1 rounded text-[#EA580C]">public/gallery/smokers-pellet/</code> to see them here.</p>
           </div>
         ) : (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {galleryImages.map((imgSrc, idx) => (
                 ...
                 >
                    <Image 
                      src={imgSrc} 
                      alt={`Pellet Gallery Image ${idx + 1}`} 
                      fill 
