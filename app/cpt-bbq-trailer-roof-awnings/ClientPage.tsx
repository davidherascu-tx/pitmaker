...
const SPECS = [
  ...
  { label: "Wheels", value: "15” Wheels Standard", icon: Settings }
];
...
export default function CPTRoofAwningsClient({ galleryImages, cmsData }: { galleryImages: string[], cmsData: any }) {
  const router = useRouter();
  const [selectedOptions, setSelectedOptions] = useState<Record<string, boolean>>({});
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  const BASE_PRICE = cmsData?.basePrice || 19995;
      ...
      <section className="container mx-auto px-6 mt-32 border-t border-white/5 pt-20">
         ...
         {galleryImages.length === 0 ? (
           <div className="w-full py-20 border border-dashed border-zinc-800 rounded-[2rem] flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-xs font-light mt-2">Upload images to <code className="bg-black px-2 py-1 rounded text-[#EA580C]">public/gallery/cpt-bbq-trailer-roof-awnings/</code></p>
           </div>
         ) : (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {galleryImages.map((imgSrc, idx) => (
                 ...
                 >
                    <Image 
                      src={imgSrc} 
                      alt={`CPT Trailer Gallery Image ${idx + 1}`} 
                      fill 
