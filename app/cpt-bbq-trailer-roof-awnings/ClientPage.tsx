...
import { 
  ...
  Flame, Zap, Music, ArrowRight, Camera, Plus, X, ChevronLeft, ChevronRight, MonitorPlay, Maximize, Lightbulb
} from "lucide-react";

// --- 1. CPT ROOF & AWNINGS TRAILER DATA ---
const BASE_PRICE = 19995;
const STOCK_NUMBER = "CPT-2-AXLE-W-RF"; 

const SPECS = [
  { label: "Length", value: "19'-1\" Overall", icon: Ruler },
  { label: "Width", value: "80\" Fender-to-Fender", icon: Ruler },
  { label: "Axle", value: "Dual 3500 lbs", icon: Truck },
  { label: "Wheels", value: "15” 10-Ply Upgraded", icon: Settings }
];
...
const STANDARD_FEATURES = [
  ...
  "Electric Overhead LED Lighting system for nighttime cooking.",
  "Two (2) Stainless Steel Food Preparation Tables w/ Lockable Dry Storage Boxes.",
  ...
  "“Bull Dog” Style Hitch with Electric Brakes Standard.",
  "LED Taillights w/ fully enclosed wiring and reinforced solid steel tube bracket."
];

const CUSTOM_OPTIONS = [
  { id: "two-tone", label: "Two Tone Frame, Roof & Boxes", price: 500, icon: Settings, desc: "Frame/Roof one Color w/ Boxes and Equipment a Different Color." },
  { id: "extra-length", label: "Add Extra 12″ To Trailer Length", price: 1400, icon: Maximize, desc: "Because sometimes you just need a little more room under the roof!" },
  { id: "extra-table", label: "Extra 58” L x 30” W Stainless Table", price: 795, icon: Hammer, desc: "Includes lockable dry storage box underneath." },
  ...
  { id: "mvp", label: "24″ x 20″ MVP Tailgater Grill", price: 895, icon: Flame, desc: "Standard carbon steel Tailgater grill mounted to your rig." },
  ...
  { id: "electrical", label: "Electrical Outlets on Frame", price: 550, icon: Zap, desc: "Fully integrated wiring with 3 all-weather outdoor power outlets." },
  { id: "stereo", label: "Marine Outdoor Stereo w/ Speakers", price: 1200, icon: Music, desc: "Marine-grade CD/Player stereo system with built-in speakers & 12V supply." },
...
export default function CPTRoofAwningsClient({ galleryImages }: { galleryImages: string[] }) {
  ...
  const handleAddToQuote = () => {
    ...
    const newItem = {
      ...
      model: "CPT Trailer - Roof & Awnings",
      stock: STOCK_NUMBER,
      total: currentTotal,
...
<Image 
    src="/images/Custom_Trailer_Roof.webp" 
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6 self-start">
            <div className="inline-flex items-center gap-2 bg-[#EA580C] text-black px-4 py-1.5 rounded-full shadow-lg">
               <ShieldCheck size={14} />
               <span className="text-[10px] font-black uppercase tracking-widest">Premium Series</span>
            </div>
          ...
          <h1 className="font-oswald text-5xl md:text-7xl font-black text-white uppercase leading-[0.9] mb-6">
            CPT Trailer <br /> <span className="text-zinc-500">Roof & Awnings</span>
          </h1>
          <p className="text-zinc-400 text-lg md:text-xl font-light leading-relaxed mb-8 max-w-xl">
            Our most versatile custom BBQ Trailer system. We add a steel framed roof and fold-out awnings to give you a true all-weather solution to outdoor cooking. Complete with standard overhead LED lighting for nighttime cooking and massive surface area for displaying your team logos.
          </p>
          ...
          <div className="lg:col-span-4 sticky top-32 z-20">
             <div className="bg-[#111111] rounded-[2.5rem] border border-white/10 shadow-2xl p-8 overflow-hidden relative">
                ...
                <div className="flex justify-between items-center mb-4 text-sm text-zinc-300">
                  <span>Base CPT w/ Roof & Awnings</span>
                ...
                <p className="text-[9px] text-zinc-600 uppercase tracking-widest text-center mt-4 font-bold">
                  *Please allow 12 to 18 weeks for fabrication.
                </p>
             </div>
          </div>
        </div>
      ...
      <section className="container mx-auto px-6 mt-32 border-t border-white/5 pt-20">
         <div className="flex items-center justify-between mb-12">
            <div>
               ...
               <p className="text-zinc-500 font-light text-sm">See the CPT Roof & Awnings in action.</p>
            </div>
         ...
         {galleryImages.length === 0 ? (
           <div className="w-full py-20 border border-dashed border-zinc-800 rounded-[2rem] flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-xs font-light mt-2">Upload images to <code className="bg-black px-2 py-1 rounded text-[#EA580C]">public/gallery/cpt-bbq-trailer-roof-awnings/</code> to see them here.</p>
           </div>
         ) : (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {galleryImages.map((imgSrc, idx) => (
                 ...
                 >
                    <Image 
                      src={imgSrc} 
                      alt={`CPT Roof & Awnings Gallery Image ${idx + 1}`} 
                      fill 
