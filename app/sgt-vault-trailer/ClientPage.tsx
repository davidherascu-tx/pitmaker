...
export default function SGTVaultClient({ galleryImages, cmsData }: { galleryImages: string[], cmsData: any }) {
  ...
  const CUSTOM_OPTIONS = rawOptions.map((opt: any, index: number) => {
    const icons = [Settings, Hammer, Flame, Flame, ShieldCheck, Zap, Music];
    return {
      id: `opt-${index}`,
      label: opt.label,
      price: opt.price || 0,
