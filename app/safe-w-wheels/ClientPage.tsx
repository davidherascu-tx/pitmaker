...
const STOCK_NUMBER = "PM-BBQ-SAFE-W-WHEELS"; 

const SPECS = [
  { label: "Dimensions", value: "68″ H x 22″ W x 31.5″ D", icon: Ruler },
  { label: "Grate Area", value: "1230 Sq. In.", icon: LayoutDashboard },
  { label: "Capacity", value: "4 Briskets", icon: Flame },
  { label: "Weight", value: "400 lbs", icon: Settings }
];

const STANDARD_FEATURES = [
  "Stainless Steel Handle & Slam-Latches on Cooking Chamber and Firebox.",
  ...
  "Heavy Duty Slide-Out Firegrate and Slide-Out Cooking Grates.",
  "Stainless Steel Smoke Stack and Smoke Stack Damper.",
  "Solid Stainless Steel Drip Pan/Top of Firebox for extra durability and corrosion resistance.",
  ...
  "Heavy Duty 4-1/2″ x 2″ and 10″ x 2″ Solid Rubber Casters w/ grease bearings for Easy Moving."
];

const CUSTOM_OPTIONS = [
  { id: "all-stainless", label: "Solid Stainless Option", price: 5800, icon: ShieldCheck, desc: "Upgrade the entire body to solid stainless steel." },
  ...
  { id: "stainless-grates", label: "Solid Stainless Cooking Grates", price: 800, icon: ShieldCheck, desc: "Upgrade to all Solid Stainless Steel Cooking Grates for high corrosion environments." }
];

export default function SafeWheelsClient({ galleryImages }: { galleryImages: string[] }) {
  ...
  const toggleOption = (id: string) => {
    setSelectedOptions(prev => {
      ...
      // Handle mutually exclusive ash pans
    ...
    <main className="min-h-screen bg-[#0a0a0a] selection:bg-[#EA580C] pt-24 pb-48 font-sans">
      ...
      <section className="relative container mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
        <div className="w-full lg:w-1/2 relative aspect-[4/3] rounded-[2.5rem] md:rounded-[3.5rem] bg-[#111111] border border-white/10 overflow-hidden shadow-2xl group">
          ...
          <Image 
            src="/images/smokers_safe_w_wheels.webp" 
            alt="Safe w/ Wheels" 
            fill 
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center">
          <div className="flex flex-wrap items-center gap-4 mb-6 self-start">
            <div className="inline-flex items-center gap-2 bg-[#EA580C] text-black px-4 py-1.5 rounded-full shadow-lg">
               <Flame size={14} />
               <span className="text-[10px] font-black uppercase tracking-widest">Safe Series</span>
            </div>
          ...
          <h1 className="font-oswald text-5xl md:text-7xl font-black text-white uppercase leading-[0.9] mb-6">
            Safe <br /> <span className="text-zinc-500">w/ Wheels</span>
          </h1>
          <p className="text-zinc-400 text-lg md:text-xl font-light leading-relaxed mb-8 max-w-xl">
            The BBQ Safe has all of the same awesome characteristics of the much larger BBQ Vault. With the same 3/4″ high temperature gasket and insulation, it is incredibly efficient. Lock in the Flavor!™ with zero smoke leaks.
          </p>
      ...
      <section className="container mx-auto px-6 mt-32 border-t border-white/5 pt-20">
         <div className="flex items-center justify-between mb-12">
            <div>
               ...
               <p className="text-zinc-500 font-light text-sm">See the Safe w/ Wheels in action.</p>
            </div>
         ...
         {galleryImages.length === 0 ? (
           <div className="w-full py-20 border border-dashed border-zinc-800 rounded-[2rem] flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-xs font-light mt-2">Upload images to <code className="bg-black px-2 py-1 rounded text-[#EA580C]">public/gallery/safe-w-wheels/</code></p>
           </div>
         ) : (
