...
const STANDARD_FEATURES = [
  "Mounted with single PM-BBQ-Vault Smoker.",
...
export default function LTVaultClient({ galleryImages, cmsData }: { galleryImages: string[], cmsData: any }) {
  const router = useRouter();
  const [selectedOptions, setSelectedOptions] = useState<Record<string, boolean>>({});
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  const BASE_PRICE = cmsData?.basePrice || 8995;
      ...
      <section className="relative container mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
        <div className="w-full lg:w-1/2 relative aspect-[4/3] rounded-[2.5rem] md:rounded-[3.5rem] bg-[#111111] border border-white/10 overflow-hidden shadow-2xl group">
          ...
          <Image 
            ...
            alt="Pitmaker LT Trailer w/ Vault" 
            fill 
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center">
          ...
          <p className="text-zinc-400 text-lg md:text-xl font-light leading-relaxed mb-8 max-w-xl">
            Compact, rugged, and exceptionally efficient. This LT Trailer is outfitted with the iconic BBQ Vault Smoker. The vertical cabinet design means you can fit massive amounts of meat on a single axle frame without sacrificing fuel efficiency or heavy smoke flavor.
          </p>
      ...
      <section className="container mx-auto px-6 mt-32 border-t border-white/5 pt-20">
         ...
         {galleryImages.length === 0 ? (
           <div className="w-full py-20 border border-dashed border-zinc-800 rounded-[2rem] flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-xs font-light mt-2">Upload images to <code className="bg-black px-2 py-1 rounded text-[#EA580C]">public/gallery/lt-trailer-w-bbq-vault/</code></p>
           </div>
         ) : (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {galleryImages.map((imgSrc, idx) => (
                 ...
                 >
                    <Image 
                      src={imgSrc} 
                      alt={`LT Vault Trailer Gallery Image ${idx + 1}`} 
                      fill 
