...
import { 
  ...
  Flame, ArrowRight, Camera, Plus, X, ChevronLeft, ChevronRight, LayoutDashboard, Droplet
} from "lucide-react";

const BASE_PRICE = 4495;
const STOCK_NUMBER = "PM-BBQ-VAULT"; 

const SPECS = [
  { label: "Dimensions", value: "77″ H x 32″ W x 28.5″ D", icon: Ruler },
  { label: "Grate Area", value: "2750 Sq. In.", icon: LayoutDashboard },
  { label: "Capacity", value: "12-15 Briskets", icon: Flame },
  { label: "Weight", value: "925 lbs", icon: Settings }
];

const STANDARD_FEATURES = [
  "Double-walled inside and out, 1/8″ thick virgin steel, fully welded and seamless.",
  "High-tech ceramic wool insulation on all six sides of the smoker.",
  "14 Ga. Solid Stainless Steel Insulated Cooking Chamber Doors and Firebox Doors.",
  "7/8″ High Temperature Gasket on firebox and cooking chamber doors.",
  "Solid Stainless 1″ x 1″ Tubular Handles for hanging towels, tools, etc.",
  "Heavy Duty Slide-Out Firegrate and 5 Slide-Out Cooking Grates.",
  "Solid Stainless Steel Drip Pan for extra durability and corrosion resistance.",
  "1-1/4″ Heavy Duty Ball Valve Drain for Water Reservoir/Drip Pan.",
  "2″ Ball Valve Drain for Air Intake Damper for Precision Oxygen Control.",
  "Stainless Steel Smoke Stack, Damper, and Hinges for extreme corrosion resistance.",
  "Color Paint Choice with Zinc Epoxy Primer Included."
];

const CUSTOM_OPTIONS = [
  { id: "all-stainless", label: "All Solid Stainless Steel", price: 9500, icon: ShieldCheck, desc: "Make your entire Vault out of solid stainless steel (Includes grates & firegrate)." },
  { id: "stainless-grates", label: "Solid Stainless Cooking Grates", price: 950, icon: ShieldCheck, desc: "Upgrade all 5 grates to solid stainless steel (for Commercial Application)." },
  { id: "carbon-ash", label: "Carbon Ash Pan", price: 110, icon: Hammer, desc: "Custom carbon steel ash pan for easy firebox cleanout." },
  { id: "stainless-ash", label: "Stainless Ash Pan", price: 220, icon: ShieldCheck, desc: "Premium solid stainless steel ash pan." },
  { id: "extra-grate", label: "Extra Cooking Grate (Installed)", price: 235, icon: LayoutDashboard, desc: "Add an extra cooking grate with additional installed channel." },
  ...
  { id: "deflector", label: "Deflector/Diverter Plate", price: 70, icon: Settings, desc: "Plate for inside the firebox to slow evaporation during water cooking." }
];

export default function VaultClient({ galleryImages }: { galleryImages: string[] }) {
    ...
    <main className="min-h-screen bg-[#0a0a0a] selection:bg-[#EA580C] pt-24 pb-48 font-sans">
      ...
      <section className="relative container mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
        <div className="w-full lg:w-1/2 relative aspect-[4/3] rounded-[2.5rem] md:rounded-[3.5rem] bg-[#111111] border border-white/10 overflow-hidden shadow-2xl group">
          ...
          <Image 
            src="/images/smoker_vault.webp" 
            alt="Pitmaker Vault Smoker" 
            fill 
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center">
          <div className="flex flex-wrap items-center gap-4 mb-6 self-start">
            <div className="inline-flex items-center gap-2 bg-[#EA580C] text-black px-4 py-1.5 rounded-full shadow-lg">
               <Flame size={14} />
               <span className="text-[10px] font-black uppercase tracking-widest">Flagship Series</span>
            </div>
          ...
          <h1 className="font-oswald text-5xl md:text-7xl font-black text-white uppercase leading-[0.9] mb-6">
            The <br /> <span className="text-zinc-500">Vault</span>
          </h1>
          <p className="text-zinc-400 text-lg md:text-xl font-light leading-relaxed mb-8 max-w-xl">
            The most durable vertical, insulated BBQ Smoker ever made. Lock in the Flavor!™ with zero smoke leaks, pure efficiency, and a massive 2750 square inch cooking capacity. 
          </p>
      ...
      <section className="container mx-auto px-6 mt-32 border-t border-white/5 pt-20">
         <div className="flex items-center justify-between mb-12">
            <div>
               ...
               <p className="text-zinc-500 font-light text-sm">See the legendary Vault in action.</p>
            </div>
         ...
         {galleryImages.length === 0 ? (
           <div className="w-full py-20 border border-dashed border-zinc-800 rounded-[2rem] flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-xs font-light mt-2">Upload images to <code className="bg-black px-2 py-1 rounded text-[#EA580C]">public/gallery/smokers-vault/</code></p>
           </div>
         ) : (
