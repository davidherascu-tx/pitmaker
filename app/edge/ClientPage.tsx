...
export default function EdgeClient({ galleryImages, cmsData }: { galleryImages: string[], cmsData: any }) {
  const router = useRouter();
  const [selectedOptions, setSelectedOptions] = useState<Record<string, boolean>>({});
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  const BASE_PRICE = cmsData?.basePrice || 5795;
  const rawOptions = cmsData?.options || [];
  
  const CUSTOM_OPTIONS = rawOptions.map((opt: any, index: number) => {
    const icons = [ShieldCheck, Hammer, LayoutDashboard, Settings];
    return {
      id: `opt-${index}`,
      label: opt.label,
      price: opt.price || 0,
  ...
  const toggleOption = (option: any) => {
    setSelectedOptions(prev => {
      ...
      if (isSelected) {
        ...
        // Group Logic for mutually exclusive options
        if (option.group) {
