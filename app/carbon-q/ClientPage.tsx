...
import { 
  ...
  Flame, ArrowRight, Camera, Plus, X, ChevronLeft, ChevronRight, LayoutDashboard, Target
} from "lucide-react";

const STOCK_NUMBER = "PM-CQ-30"; 

const SPECS = [
  { label: "Grate #1", value: "26” x 22”", icon: Ruler },
  { label: "Grate #2", value: "26” x 16”", icon: Ruler },
  { label: "Total Area", value: "713 Sq. In.", icon: LayoutDashboard },
  { label: "Design", value: "Sealed Heat Broiler", icon: Target }
];

const STANDARD_FEATURES = [
  "1/8” Plate steel body and lid construction.",
  "Stainless steel hinges and cool-to-touch lid handle.",
  "Sliding air damper for precision air intake control.",
  "Adjustable charcoal grill basket.",
  "Slide-out removable ash pan.",
  "Two Slide-out cooking grates.",
  "Baffle outtake and Stainless steel smoke stack.",
  "Extremely efficient burn time (350 degrees for 1.5 hrs on 5 lbs of charcoal)."
];

export default function CarbonQClient({ galleryImages, cmsData }: { galleryImages: string[], cmsData: any }) {
  const router = useRouter();
  const [selectedOptions, setSelectedOptions] = useState<Record<string, boolean>>({});
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  const BASE_PRICE = cmsData?.basePrice || 2895;
  const rawOptions = cmsData?.options || [];
  
  const CUSTOM_OPTIONS = rawOptions.map((opt: any, index: number) => {
    const icons = [Flame, Target, ShieldCheck, Settings, Hammer];
    return {
      id: `opt-${index}`,
      label: opt.label,
      price: opt.price || 0,
      ...
      requiresQuote: opt.requiresQuote || false, // <--- GRABS THE CHECKBOX FROM SANITY
  ...
  const handleAddToQuote = () => {
    const activeOptions = CUSTOM_OPTIONS.filter((o: any) => selectedOptions[o.id]).map((o: any) => ({
      label: o.label,
      price: o.requiresQuote ? "Quote" : o.price // Send "Quote" text to backend cart
    }));
    ...
    <main className="min-h-screen bg-[#0a0a0a] selection:bg-[#EA580C] pt-24 pb-48 font-sans">
      ...
      <section className="relative container mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
        <div className="w-full lg:w-1/2 relative aspect-[4/3] rounded-[2.5rem] md:rounded-[3.5rem] bg-[#111111] border border-white/10 overflow-hidden shadow-2xl group">
          ...
          <Image 
            src="/images/grill_carbon_q.webp" 
            alt="Pitmaker Carbon-Q Grill" 
            fill 
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center">
          <div className="flex flex-wrap items-center gap-4 mb-6 self-start">
            <div className="inline-flex items-center gap-2 bg-[#EA580C] text-black px-4 py-1.5 rounded-full shadow-lg">
               <Target size={14} />
               <span className="text-[10px] font-black uppercase tracking-widest">Competition Series</span>
            </div>
          ...
          <h1 className="font-oswald text-5xl md:text-7xl font-black text-white uppercase leading-[0.9] mb-6">
            Carbon-Q <br /> <span className="text-zinc-500">Grill</span>
          </h1>
          <p className="text-zinc-400 text-lg md:text-xl font-light leading-relaxed mb-8 max-w-xl">
            A “sealed heat” charcoal grill with precision control over air intake. Made to cook with an omni-directional convection heat, it's known in the competition world as the ultimate “chicken cooker”.
          </p>
                ...
                <div className="border-t border-white/10 pt-6 mb-8 flex flex-col gap-1">
                   <div className="flex justify-between items-end gap-4">
                     ...
                     <span className="font-oswald text-4xl md:text-5xl font-black text-[#EA580C] tracking-tighter shrink-0">
                       {/* DYNAMIC TOTAL RENDER (Adds the + sign) */}
      ...
      <section className="container mx-auto px-6 mt-32 border-t border-white/5 pt-20">
         <div className="flex items-center justify-between mb-12">
            <div>
               ...
               <p className="text-zinc-500 font-light text-sm">See the Carbon-Q in action.</p>
            </div>
         ...
         {galleryImages.length === 0 ? (
           <div className="w-full py-20 border border-dashed border-zinc-800 rounded-[2rem] flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-xs font-light mt-2">Upload images to <code className="bg-black px-2 py-1 rounded text-[#EA580C]">public/gallery/grills-carbon-q/</code></p>
           </div>
         ) : (
