...
import { 
  ...
  UploadCloud, File as FileIcon, X, Trash2, Loader2
} from "lucide-react";
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Manually append the file if one is selected
    if (selectedFile) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 2. USE THE SAVED REFERENCE TO RESET
        formElement.reset(); 
        setSelectedFile(null);
      } else {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 relative">
      ...
      <div className="lg:col-span-7">
        ...
        {submitStatus === "success" ? (
          <div className="bg-green-500/10 border border-green-500/50 rounded-2xl p-8 text-center flex flex-col items-center">
            <CheckCircle2 size={48} className="text-green-500 mb-4" />
            <h3 className="text-2xl font-oswald font-bold text-white uppercase mb-2">Message Sent!</h3>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <input type="text" name="firstName" required className="w-full bg-[#111111] border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-[#EA580C] focus:ring-1 focus:ring-[#EA580C] transition-all" placeholder="John" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest pl-1">Last Name</label>
                  <input type="text" name="lastName" required className="w-full bg-[#111111] border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-[#EA580C] focus:ring-1 focus:ring-[#EA580C] transition-all" placeholder="Doe" />
                </div>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <input type="email" name="email" required className="w-full bg-[#111111] border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-[#EA580C] focus:ring-1 focus:ring-[#EA580C] transition-all" placeholder="john@example.com" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest pl-1">Phone Number</label>
                  <input type="tel" name="phone" className="w-full bg-[#111111] border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-[#EA580C] focus:ring-1 focus:ring-[#EA580C] transition-all" placeholder="(555) 123-4567" />
                </div>
             </div>
             ...
             {cartItems.length > 0 && <input type="hidden" name="cart_data" value={JSON.stringify(cartItems)} />}
             ...
             {submitStatus === "error" && (
               <p className="text-red-500 text-sm font-bold">An error occurred. Please try again or call us.</p>
             )}

             <button disabled={isSubmitting} type="submit" className="w-full group relative overflow-hidden bg-[#EA580C] text-black px-6 py-5 rounded-xl font-black uppercase tracking-widest text-sm hover:scale-[1.01] transition-all shadow-[0_0_20px_rgba(234,88,12,0.4)] mt-4 disabled:opacity-70 disabled:hover:scale-100 cursor-pointer disabled:cursor-not-allowed">
                ...
                {!isSubmitting && <div className="absolute inset-0 bg-white/40 skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-500" />}
             </button>
          </form>
        )}
      </div>
